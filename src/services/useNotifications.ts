...
import {
  mapApiNotificationToClient,
  parseNotificationsListResponse,
  parseUnreadCountResponse,
} from "@/lib/mapClientNotification";
import type { Notification } from "@/types/notifications";
import type { NotificationsPage } from "@/types/client-notifications";
...
async function fetchNotificationsPage(params: {
  ...
  const parsed = parseNotificationsListResponse(unwrapPayload(data));
  return parsed;
}
...
export function useMarkNotificationRead() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      return mapApiNotificationToClient(unwrapPayload(data) ?? { id, isRead: true });
    },
    onMutate: async (id) => {
