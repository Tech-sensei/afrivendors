...
import { useOpenAppointmentDispute } from "@/services/useAppointments";
...
export function OpenAppointmentDisputeDialog({
  appointment,
  open,
  onOpenChange,
  onSubmitted,
  ...
  const { mutate: submitDispute, isPending } = useOpenAppointmentDispute();
  ...
  const handleSubmit = () => {
    if (!appointment || trimmedReason.length < MIN_REASON_LENGTH) return;
      ...
      <AlertDialogContent className="max-w-md rounded-2xl border-accent-20">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-left text-sm leading-relaxed text-accent-80">
            {appointment ? (
              <>
                Payment for your booking with{" "}
                <span className="font-semibold text-secondary-000">
                  {vendorName}
                </span>{" "}
                ({formatVendorPrice(appointment.totalAmount)}) will stay on
                hold while Afrivendors reviews your case. Release funds will be
                disabled until this is resolved.
              </>
            ) : (
              "Tell us what went wrong with your completed booking."
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
