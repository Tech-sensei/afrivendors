...
const VendorCard = ({
  ...
  return (
    ...
    >
      <Card
        className="h-full flex flex-col cursor-pointer transition-all duration-300 hover:scale-[1.02] overflow-hidden group py-0 rounded-2xl border border-[#EFE6E1] shadow-[0_4px_12px_rgba(35,19,5,0.08)]"
        ...
        <div className="relative h-56 overflow-hidden">
          ...
          {onFavouriteToggle && (
            <button
              ...
              aria-busy={favouritePending}
              className="absolute top-4 right-4 h-10 w-10 rounded-full flex items-center justify-center transition-all duration-200 bg-white/90 hover:bg-white border-none cursor-pointer z-10 disabled:opacity-80 disabled:cursor-wait"
              onClick={(e) => {
                e.stopPropagation();
                if (!favouritePending) onFavouriteToggle(vendor.id, !isFavourite);
              }}
              aria-label={isFavourite ? "Remove from favourites" : "Add to favourites"}
            >
              {favouritePending ? (
                <Loader2 className="h-5 w-5 animate-spin text-primary-100" aria-hidden />
              ) : (
                <Heart
