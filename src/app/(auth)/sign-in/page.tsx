...
const SignInPage = () => {
    ...
    const handleContinue = () => {
        if (email && agreedToTerms) {
            ...
            const isAdminEmail = email.toLowerCase() === 'admin@afrivendor.co.uk';
            const isCustomerEmail = email.toLowerCase() === 'amara@gmail.com';

            if (isAdminEmail) {
                // If admin, go to admin dashboard
                router.push('/admin-dashboard');
            } else if (isCustomerEmail) {
                // If registered customer, go to customer dashboard
                // router.push('/dashboard-profile');
                toast.success('Welcome to the customer dashboard');
            } else {
                // If not registered, go to registration page
                router.push(`/sign-up?type=customer&email=${encodeURIComponent(email)}`);
            }
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 min-h-screen max-h-screen overflow-hidden">
                ...
                <div className="flex flex-col justify-between p-6 md:p-8 overflow-y-auto">
                    <div className="max-w-[560px] w-full mx-auto">
                        {/* Back Button */}
                        <button
                            onClick={() => router.push('/sign-up-choice')}
